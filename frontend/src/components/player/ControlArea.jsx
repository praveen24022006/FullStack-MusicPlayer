...
const ControlArea = ({playerState,  playerControls}) => {
  ...
  const isLiked = Boolean(currentSongId && user?.favourites?.some((fav)=> fav.id===currentSong.id),
  );
  ...
  const handleLike = async () =>{
    ...
    try {
      ...
      const res = await axios.post(`${import.meta.env.VITE_BASE_URL}/api/songs/favourite`,
        {song:songData},
    ...
    <div className="control-root">
      ...
      <div className="control-progress-wrapper">
        <input
          ...
          style={{
            background:`linear-gradient(to right, #a855f7 ${duration ? (currentTime/duration)*100:0}%,#333 ${duration? (currentTime/
              duration)*100:0}%)`,
          }}
        />
